...
set "AMENT_ENVIRONMENT_HOOKS[new_package]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[new_package] "%AMENT_CURRENT_PREFIX%\share\new_package\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[new_package] "%AMENT_CURRENT_PREFIX%\share\new_package\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[new_package]:;=";"%") do (
    call:call_file %%f
  )
